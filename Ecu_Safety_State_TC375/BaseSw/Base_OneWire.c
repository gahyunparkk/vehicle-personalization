...
static volatile uint8 m_busy_line = 0;

#define ONEWIRE_BUSY_ON() \
    do { \
        m_busy_line = 1U; \
    } while (0)


#define ONEWIRE_BUSY_OFF() \
    do { \
        m_busy_line = 0U; \
    } while (0)
...
{
    // 현재 시점부터 time_us에 해당하는 tick만큼 blocking wait
...
void OneWire_TargetSetup(OneWire_t* OneWireStruct, uint8 family_code) {
   uint8 i;
...
uint8 OneWire_GetROM(OneWire_t* OneWireStruct, uint8 index) {
